...
spec:
  pipeline:
    params:
      - name: ociStorage
        value: quay.io/my-org/my-component-release-ta
```

For local Kind deployments using the internal registry:

```yaml
        value: registry-service.kind-registry/test-component-release-ta
```
...
5. Click **Generate Access Token → Authorize Application** and copy the token. This
